...
export default function DriverDashboard() {
  ...
  const startLive = useCallback(() => {
    ...
    if ('geolocation' in navigator) {
      navigator.geolocation.getCurrentPosition(
        (pos) => {
          // Real GPS available - use it
          setSimulationMode(false)
          const lat = pos.coords.latitude
          const lng = pos.coords.longitude
          const speed = pos.coords.speed ? Math.round(pos.coords.speed * 3.6) : null // m/s to km/h
    ...
    intervalRef.current = setInterval(() => {
      if (simulationMode || !('geolocation' in navigator)) {
        ...
        simIndexRef.current = (simIndexRef.current + 1) % (routeCoords.length * 2) // loop back
        const progress = simIndexRef.current / 2 // half-step for smoothness
        ...
        navigator.geolocation.getCurrentPosition(
          ...
          () => {
            // GPS failed on subsequent call - switch to simulation
            setSimulationMode(true)
          },
          { enableHighAccuracy: true, timeout: 5000 }
        )
  ...
  }, [bus, currentUser, setIsDriverLive, simulationMode, routeCoords])

  const stopLive = useCallback(() => {
    if (socketRef.current) {
      socketRef.current.emit('driver-stop', {
        busId: bus?.id,
        ownerId: bus?.ownerId,
      })
  ...
  }, [bus?.id, bus?.ownerId, setIsDriverLive])
        ...
        <Card className={`rounded-2xl border-0 shadow-sm ${isDriverLive ? 'bg-emerald-50' : 'bg-gray-50'}`}>
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              ...
              >
                {isDriverLive ? 'Stop Sharing' : 'Go Live'}
              </Button>
            </div>
